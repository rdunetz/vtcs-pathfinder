...
function validateNestedPrereqs(prereqs) {
  ...
  return prereqs.every((group) => {
    ...
    return Array.isArray(group) && group.every((course) => typeof course === "string");
  });
}
...
async function seed() {
  ...
  for (const course of courses) {
    ...
    // Normalize prerequisites to nested array format
    // Note: Firestore doesn't support nested arrays natively, so we'll store as JSON string
    const prerequisites = normalizePrerequisites(course.prerequisites);
    
    // Convert nested array to JSON string for Firestore storage
    const prerequisitesData = prerequisites.length > 0 ? JSON.stringify(prerequisites) : null;
    ...
    const baseData = {
      ...
      prerequisites: prerequisitesData, // Stored as JSON string
      corequisites,
    ...
    if (!snap.exists) {
      ...
      console.log(`🆕 Created ${course.code}: ${course.name} | prereqs: ${prerequisitesData || "none"}`);
    } else {
      // Existing document: DO NOT modify createdAt
      await docRef.set(baseData, { merge: true });
      console.log(`🔁 Updated ${course.code}: ${course.name} | prereqs: ${prerequisitesData || "none"}`);
    }
  }
